...
using namespace std;
const int NN=1005;
int boy_start[NN][NN],girl_start[NN][NN],boy_end[NN][NN],girl_end[NN][NN],calorie[NN][NN];
int maxcalories(int M,int N){
    ...
    for (int i = 2; i < M; i++)
    {
        for (int j = 2; j < N; j++)
        {
            ...
            int ans2=boy_start[i-1][j]+boy_start[i+1][j]+girl_start[i][j-1]+girl_end[i][j+1];
