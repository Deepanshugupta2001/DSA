...
bool sudokuSolver(int i,int j,int n,int mat[][9]){
    ...
    //We are increasing j everytime,toh j bhi 
    if(j==n){
        return sudokuSolver(i+1,0,n,mat);
    }
    //Agar cell already filled hai
    if(mat[i][j]!=0){
        //Ab jo bhi answer recusion dega choti
